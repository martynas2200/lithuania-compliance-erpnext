...
function getItemPricesAndOpenDialog(frm, forced = false) {
    ...
    frappe.call({
        ...
        callback: function (r) {
            if (r.message) {
                show_manage_prices_dialog(frm, r.message, forced);
            }
        },
    });
}
...
function show_manage_prices_dialog(frm, items_data, forced = false) {
    // Filter items: keep only those with no applicable price
    // or with markup below 20%
    const filtered_items = (items_data || []).filter((item) => {
        // Really specific for Lithuania
        // Would be a good idea to make this configurable later
        if (item.item_name === "Užstatinė tara") {
            return false;
        }
        const has_no_price = !item.applicable_price;
        const markup_value = typeof item.markup === "number" ? item.markup : null;
        const has_low_markup = markup_value !== null && markup_value < 20;
        return has_no_price || has_low_markup;
    });

    if (!forced && !filtered_items.length) {
        return;
    }
